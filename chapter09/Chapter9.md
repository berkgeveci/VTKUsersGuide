...
> **Note:** The high-level geospatial visualization classes described in Sections 9.1–9.4 (`vtkGeoView`, `vtkGeoView2D`, `vtkGeoTerrain`, `vtkGeoSource`, `vtkGeoGraticule`, and related classes) were deprecated in VTK 8.2 and removed in VTK 9.0. Only `vtkGeoTransform` and `vtkGeoProjection` (Section 9.5) remain in current VTK. The code examples in this chapter are preserved for historical reference but will not compile or run with modern VTK.

Geospatial visualization is a discipline that combines techniques from traditional scientific visualization and information visualization in order to display geographically organized data.

## 9.1 Geographic Views and Representations

The main geospatial visualization functionality provided by VTK is the ability to render extremely large texture images onto geometric representations of the Earth. Both the image data and the geometry it is textured upon must be hierarchical representations that are loaded on demand from disk or a network server because they would otherwise consume too much memory and take too long to load. The following Python snippet shows how to display a map in VTK.
...
```python
# NOTE: vtkGeoView was removed in VTK 9.0.
...
rd = vtkJPEGReader()
rd.SetFileName("NE2_ps_bath_small.jpg")
rd.Update()
...
gv.GetRenderer().SetBackground2(0.5, 0.5, 0.5)
...
GeoView's `AddDefaultImageRepresentation` method creates two representations: one—vtkGeoAlignedImageRepresentation—that takes a high resolution input map image and generates a tile hierarchy; and one—vtkGeoTerrain—that generates a geometry hierarchy representing a 3-dimensional globe. You are of course free to use different representations. The next two paragraphs show how to manually create and add representations.

The vtkGeoView will accept one geometry representation and multiple aligned image representations. For instance, by adding the following lines to the script above you can display a cloud layer on top of the base map.

```python
# NOTE: vtkGeoAlignedImageRepresentation/Source were removed in VTK 9.0.

# Read in a cloud cover image
rd2 = vtkJPEGReader()
rd2.SetFileName("clouds.jpeg")
rd2.Update()
...
To add geographically located information to the visualization, vtkGeoView will accept graph representations whose coordinates are specified as latitude and longitude.
...
```python
# NOTE: vtkGeoGraphRepresentation and vtkGeoRandomGraphSource
# were removed in VTK 9.0.
...
```python
# NOTE: vtkGeoView2D, vtkGeoProjectionSource, and vtkGeoTerrain2D
# were removed in VTK 9.0.
...
- alternative sources named vtkGeoFileImageSource and vtkGeoFileTerrainSource can be used which read these tiles from disk instead of generating them on the fly from the source image or cartographic projection.
...
```python
# NOTE: vtkGeoGraticule was removed in VTK 9.0.
# vtkGeoProjection and vtkGeoTransform still exist in current VTK.

# The default is latlong (no projection at all)
ps = vtkGeoProjection()
...
# The vtkTransformFilter uses a transform to map points
